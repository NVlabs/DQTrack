...
class UVTR(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        if self.with_img_backbone:
            ...
            elif depth_head.type == "StackConvDepth":
                self.depth_dim = depth_head.model.depth_dim
                kernel_size = depth_head.model.kernel
                padding = (kernel_size-1)//2
                ops = OrderedDict()
                for k in range(depth_head.model.num_conv):
                    ops['conv_{}'.format(k)] = nn.Conv2d(out_channels,
                                                        out_channels,
                                                        kernel_size=kernel_size,
                                                        padding=padding,
                                                        bias=True)
                    if depth_head.model.get('with_bn', True):
                        ops['bn_{}'.format(k)] = nn.BatchNorm2d(out_channels)
                    if depth_head.model.get('with_relu', True):
                        ops['relu_{}'.format(k)] = nn.ReLU(inplace=True)
                ops['conv_out'] = nn.Conv2d(out_channels, self.depth_dim, kernel_size=1)
                self.depth_net = nn.Sequential(ops)
            else:
    ...
    def pred_depth(self, img, img_metas, img_feats=None):
        ...
        if img.dim() == 5 and img.size(0) == 1:
            ...
            B, N, C, H, W = img.size()
            img = img.view(B * N, C, H, W)
        if self.depth_head.type in ["SimpleDepth", "StackConvDepth"]:
            depth = []
