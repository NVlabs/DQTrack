...
from projects.mmdet3d_plugin.models.utils.stereo_depth import DepthNet
from projects.mmdet3d_plugin.core.track.instances import Instances as Tracklet
from nuscenes.eval.detection.config import config_factory

@DETECTORS.register_module()
class StereoTrackerDQ(MVXTwoStageDetector):
    """StereoTrackerDQ."""
    def __init__(self,
                 use_grid_mask=False,
        ...
        if self.with_img_backbone:
            ...
            elif "StereoDepth" in depth_head.type:
                depth_bound = depth_head.model.depth_bound
                self.depth_net = DepthNet(depth_head.model.in_channels,
                                          depth_head.model.mid_channels,
                                          depth_head.model.out_channels,
                                          int((depth_bound[1] - depth_bound[0])//depth_bound[2]),
                                          depth_bound,
                                          depth_head.model.num_ranges,
                                          depth_head.model.get('camera_aware', False))
            else:
        ...
        self.query_embed_det = nn.Embedding(tracker_cfg.num_query, 
                                            tracker_cfg.embed_dims * 2)
        self.refer_points_det = nn.Linear(tracker_cfg.embed_dims, 3)
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        # stereo feat with downsample rate 4
        stereo_feats = img_feats[0].detach()
        
        if self.with_img_neck:
            img_feats = self.img_neck(img_feats)[0]
        
        # shape: (B*num_views, C, H, W)
        img_feats_reshaped = img_feats.reshape(-1, *img_feats.shape[-3:])
        
        return img_feats_reshaped, stereo_feats
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if hasattr(self, "img_backbone"):
            # consider camera-based setting only:
            img_feats, img_depth = {}, {}
            # shape: (B*num_frame, num_views, sweep_num, C, H, W)
            img = img.reshape(-1, self.tracker_cfg.num_cams, 
                              self.tracker_cfg.num_sweeps, *img.shape[-3:])
            for sweep_idx in range(img.shape[2]):
                if sweep_idx == 0:
                    single_feat, stereo_feat = self.extract_img_feat(img[:,:,sweep_idx], img_metas)
                    single_depth = self.pred_depth(single_feat, img_metas, sweep_idx)
                    img_feats['ref_feat'] = single_feat
                    img_feats['ref_stereo'] = stereo_feat
                    img_depth['ref_depth'] = single_depth
                else:
                    if self.tracker_cfg.get('detach_grad', True):
                        with torch.no_grad():
                            single_feat, stereo_feat = self.extract_img_feat(img[:,:,sweep_idx], img_metas)
                            single_depth = self.pred_depth(single_feat, img_metas, sweep_idx)
                    else:
                        single_feat, stereo_feat = self.extract_img_feat(img[:,:,sweep_idx], img_metas)
                        single_depth = self.pred_depth(single_feat, img_metas, sweep_idx)
                    img_feats['src_feat'] = single_feat
                    img_feats['src_stereo'] = stereo_feat
                    img_depth['src_depth'] = single_depth
        else:
        ...
        if hasattr(self, 'pts_voxel_encoder'):
            ...
            for _points in points:
                _pts_feat = self.extract_pts_feat(_points)[0]
                pts_feats.append(_pts_feat)
        else:
            pts_feats = None
    ...
    def pred_depth(self, img_feats, img_metas, sweep_idx):
        ...
        if self.depth_head.type == "SimpleDepth":
            ...
            for _feat in img_feats:
                ...
                depth.append(_depth)
        elif self.depth_head.type == "StereoDepth":
            depth = self.depth_net(img_feats, img_metas, sweep_idx)
        else:
            raise NotImplementedError
        return depth
    ...
    def embedding_interaction(self, detector):
        ...
        if self.tracker_cfg.get('with_qim', True):
            ...
            pos_embedding = detector.query_det[..., :self.tracker_cfg.embed_dims].clone()
            # query interaction
    ...
    def forward_detector(self, points, img, img_metas, detector):
        query_dim = self.query_embed_det.weight.shape[1]
        detector.ref_pts_det = self.refer_points_det(
                                detector.query_det[..., :query_dim//2])

        pts_feat, img_feats, img_depth = self.extract_feat(points=points, 
                                                           img=img, 
                                                           img_metas=img_metas)
        ref_box_sizes = None
        pred_dict = self.pts_bbox_head(
                    detector.query_det, detector.ref_pts_det, ref_box_sizes,  
                    pts_feat, img_feats, img_metas, img_depth)

        return pred_dict, detector

    def forward_single(self, pred_dict, detector, img_metas, depth_map=None, frame_idx=0):
        output_classes = pred_dict['all_cls_scores'][:,frame_idx].float()
        output_coords = pred_dict['all_bbox_preds'][:,frame_idx].float()
        last_ref_pts = pred_dict['last_ref_points'][frame_idx].float()
        ...
        detector.output_embedding = pred_dict['last_query_feat'][frame_idx].float()  # [300, feat_dim]
        ...
        if self.training:
            ...
            if depth_map is not None:
                pred_depth = pred_dict['pred_depth'][frame_idx]
                gt_depth = depth_map[0][frame_idx]
                loss_depth = self.loss.loss_depth(pred_depth, gt_depth, frame_idx,
                                                  self.depth_head.model.depth_bound)
                loss_det.update(loss_depth)
    ...
    def forward_train(self,
        ...
        if self.tracker_cfg.get('train_track_only', False):
            with torch.no_grad():
                pred_dict, detector = self.forward_detector(points, img, img_metas, detector)
        else:
            pred_dict, detector = self.forward_detector(points, img, img_metas, detector)
        ...
        for batch_idx in range(batch_num):
            ...
            pred_dict_single = {}
            for pred_key in pred_dict:
                pred_val = pred_dict[pred_key]
                if 'all' in pred_key:
                    pred_val = pred_val.reshape(len(pred_val), batch_num, num_frame, *pred_val.shape[2:])
                    pred_dict_single[pred_key] = pred_val[:,batch_idx]
                elif 'last' in pred_key or 'raw_feat' in pred_key:
                    pred_val = pred_val.reshape(batch_num, num_frame, *pred_val.shape[1:])
                    pred_dict_single[pred_key] = pred_val[batch_idx]
                else:
                    pred_dict_single[pred_key] = pred_val
            # init gt instances!
            loss_dict_single = {}
            gt_tracklet_list = []
            ...
            for frame_idx in range(num_frame):
                ...
                loss_track = self.loss.loss_track(tracker, 
                                                det2track_mat, 
                                                info_single, 
                                                zero_val=(0*self.query_embed_det.weight.sum()),
                                                frame_idx=frame_idx)
    ...
    def forward_test(self, 
        ...
        for frame_idx in range(num_frame):
            ...
            if'lidar2img' in img_metas[0]:
                img_metas_single[0]['lidar2img'] = img_metas[0]['lidar2img'][frame_idx]
            if 'img_shape' in img_metas_single[0]:
                img_metas_single[0]['img_shape'] = img_metas_single[0]['img_shape'][frame_idx]
            if 'uni_rot_aug' in img_metas_single[0]:
                img_metas_single[0]['uni_rot_aug'] = img_metas_single[0]['uni_rot_aug'][frame_idx]
            detector = self.forward_single(pred_dict, detector, img_metas_single, frame_idx=frame_idx)
    ...
    def generate_empty_detector(self):
        ...
        num_det, dim = self.query_embed_det.weight.shape  # (300, 256 * 2)
        device = self.query_embed_det.weight.device
        ...
        query_det = self.query_embed_det.weight
        detector.query_det = query_det
