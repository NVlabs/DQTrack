...
class UVTRTrackerDQ(MVXTwoStageDetector):
    """UVTRTrackerDQ."""
    def __init__(self,
                 use_grid_mask=False,
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        # shape: (B*num_frame, num_views, C, H, W)
        img = img.reshape(-1, self.tracker_cfg.num_cams, *img.shape[-3:])
        if hasattr(self, "img_backbone"):
            img_feats = self.extract_img_feat(img, img_metas)
            img_depth = self.pred_depth(img_feats=img_feats)
        else:
    ...
    @auto_fp16(apply_to=('img'))
    def pred_depth(self, img_feats=None):
    ...
    def forward_train(self,
        ...
        img_metas[0]['img_shape'] = img_metas[0]['img_shape'][0]
