...
class NuScenesTrackDataset(Custom3DDataset):
    r"""NuScenes Dataset.
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                if self.depth_root is not None:
                    depth_path = cam_info['data_path'].split('/')[-1]
                    depth_path = depth_path.replace('.jpg', '.bin')
                    depth_path = self.depth_root + depth_path
            ...
            if self.depth_root is not None:
                input_dict['depth_filename'] = depth_paths
            # use cam sweeps
            if 'cam_sweep_num' in self.modality:
                ...
                if 'cam_sweep_list' in self.modality:
                    ...
                    for cam_idx in range(len(cam_sweeps_paths)):
                        ...
                        lidar2img_sweeps_rts[cam_idx] = [lidar2img_sweeps_rts[cam_idx][i] for i in sweep_list]
                        lidar2cam_sweeps_rts[cam_idx] = [lidar2cam_sweeps_rts[cam_idx][i] for i in sweep_list]
